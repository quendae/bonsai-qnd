...
PROFILE=''; SERVER_ONLY=0
while [ $# -gt 0 ]; do
  case "$1" in --profile) PROFILE=${2:?missing profile}; shift 2 ;; --server-only) SERVER_ONLY=1; shift ;; *) echo "[ERR] Unknown option: $1" >&2; exit 2 ;; esac
done
...
set -- --alias "$QND_HARNESS_MODEL_ID" -m "${MODEL:-<model:$QND_GGUF_PATTERN>}" --host 127.0.0.1 --port 8080 -ngl "$QND_GPU_LAYERS" -fa on -c "$QND_CONTEXT" -np "$QND_PARALLEL" -t "$threads" -tb "$batch_threads" --jinja
...
printf '[INFO] Profile: %s\n[INFO] Model: %s\n[INFO] Backend: %s\n[INFO] Context: %s\n' "$QND_PROFILE_ID" "$MODEL" "$BIN" "$QND_CONTEXT"
"$BIN" "$@" & SERVER_PID=$!
...
echo "[OK] llama-server ready: http://127.0.0.1:8080"
