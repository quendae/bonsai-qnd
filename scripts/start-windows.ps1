...
$reasoningConfig=Resolve-QndReasoning -Profile $p -Level $Reasoning
...
if($reasoningConfig.Level -ne 'model-default'){
  $args+=@('--reasoning-budget',[string]$reasoningConfig.Budget)
  if($reasoningConfig.DisableThinking){$args+=@('--reasoning-format','none','--chat-template-kwargs','{"enable_thinking":false}')}
}
if($env:QND_DRY_RUN -eq '1'){Write-Output "PROFILE $($p.id)";Write-Output "BACKEND $($p.backend)";Write-Output "CONTEXT $ctx";Write-Output "REASONING $($reasoningConfig.Level)";Write-Output "BIND $Bind";Write-Output ($bin + ' ' + ($args -join ' '));exit 0}
...
Write-Host "[INFO] Profile: $($p.id)";Write-Host "[INFO] Model: $($model.FullName)";Write-Host "[INFO] Backend: $bin";Write-Host "[INFO] Context: $ctx";Write-Host "[INFO] Reasoning: $($reasoningConfig.Level)";Write-Host "[INFO] API bind: ${Bind}:8080";if($p.PSObject.Properties.Name -contains 'kv4' -and $p.kv4){Write-Host '[INFO] KV cache: q4_0'}
