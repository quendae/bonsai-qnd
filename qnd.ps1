...
function Invoke-QndScript([string]$Path,[switch]$IncludeBind,[switch]$IncludeApiBaseUrl,[switch]$IncludeServerOnly,[switch]$IncludeReasoning){
  $params=@{}
  if($Profile){$params.Profile=$Profile}
  if($null -ne $Context){$params.Context=[int]$Context}
  if($IncludeReasoning -and $Reasoning){$params.Reasoning=$Reasoning}
...
switch($Command){
  ...
  'start' { Invoke-QndScript (Join-Path $Root 'scripts\start-windows.ps1') -IncludeBind -IncludeServerOnly -IncludeReasoning }
