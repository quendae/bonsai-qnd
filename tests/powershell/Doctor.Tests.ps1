...
foreach($needle in @('-ngl 99','-c 131072','--alias bonsai-qnd','--host 127.0.0.1','--cache-type-k q4_0','--cache-type-v q4_0')) { if(-not $out.Contains($needle)){ throw "start dry-run missing $needle" } }
...
foreach($needle in @('PROFILE amd-rx6950xt','BACKEND vulkan','FAMILY bonsai2 27B','CONTEXT 131072')) { if(-not $out.Contains($needle)){ throw "doctor dry-run missing $needle" } }
