...
try {
  ...
  foreach($needle in @('bonsai-local:','contextWindow: 65536','maxTokensField: max_tokens','provider: bonsai-local')) { if(-not $s.Contains($needle)){ throw "missing $needle" } }
} finally { Remove-Item -Recurse -Force $tmp -ErrorAction SilentlyContinue }
$tmp=Join-Path ([IO.Path]::GetTempPath()) ("qnd-"+[guid]::NewGuid())
try {
  & (Join-Path $Root 'scripts\configure-harness.ps1') -Profile nvidia-rtx3060 -DshHome $tmp | Out-Null
  $f=Join-Path $tmp 'settings.yaml'; $s=Get-Content -Raw $f
  foreach($needle in @('bonsai-local:','contextWindow: 65536','maxTokensField: max_tokens','provider: bonsai-local')) { if(-not $s.Contains($needle)){ throw "RTX 3060 harness missing $needle" } }
} finally { Remove-Item -Recurse -Force $tmp -ErrorAction SilentlyContinue }
