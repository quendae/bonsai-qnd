...
$p = Get-QndProfile 'windows-cpu'
Assert-Equal $p.platform 'windows' 'Windows CPU platform'
Assert-Equal $p.family 'bonsai' 'Windows CPU family'
Assert-Equal $p.backend 'cpu' 'Windows CPU backend'
Assert-Equal $p.gpuLayers 0 'Windows CPU GPU layers'
Assert-Equal $p.context 8192 'Windows CPU default context'
if ($p.ggufPattern -notlike '*Q1_0*') { throw 'Windows CPU profile must use Q1_0.' }
