...
if(-not $out.Contains('amd-rx6950xt') -or -not $out.Contains('nvidia-rtx3060') -or -not $out.Contains('windows-cpu') -or -not $out.Contains('cpu-agent')) { throw 'profile listing incomplete' }
...
if($cpuText -match '(?i)(?:^|\s)-Profile\s+\S'){ throw 'CPU LAN BAT must not hardcode a profile' }
